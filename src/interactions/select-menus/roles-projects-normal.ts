...
exports.run = async (SuperChan: SuperChanTypes, interaction: SelectMenuInteraction) => {
    ...
    try {
        const options = SuperChan.data.roles.projects.normal.map(c => c.options);
        const member = interaction.guild?.members.cache.get(interaction.user.id);
        const index = SuperChan.data.ordinal_options.indexOf(interaction.values[0]) - 1;
        const role = interaction.guild?.roles.cache.get(SuperChan.data.roles.projects.normal[index].roleID);
        const row = new MessageActionRow().addComponents(new MessageSelectMenu().setCustomId("roles-default").setPlaceholder("Notificação de obras normais (-18)").setOptions(options));

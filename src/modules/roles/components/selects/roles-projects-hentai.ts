...
exports.run = async (Client: SuperChan, i: SelectMenuInteraction) => {
    ...
    try {
        const possibleRoles = Client.data.menuRoles.filter(f => f.id === "project-hentai");
        const selectedRole = possibleRoles[CTO(i.values[0])];
        const role = i.guild?.roles.cache.get(selectedRole.roleId);
        const member = i.guild?.members.cache.get(i.user.id);
        const row = new MessageActionRow().addComponents(new MessageSelectMenu().setCustomId("roles-projects-hentais").setPlaceholder("Selecione aqui o seu cargo desejado.").setOptions(possibleRoles.map((c, i) => { return { label: c.label, value: OTC(i), emoji: c.emoji, description: c.description } })));

...
export default async function Commands(SuperChan: SuperChanTypes, files: string[]) {
    ...
    files.forEach(file => {
        try {
            ...
            const commandFile = require(`../../interactions/commands/${file}`);
            if (!['js', 'ts'].includes(file.split('.')[1])) return;
            if (commandFile.init) commandFile.init(SuperChan);

            SuperChan.interactions.commands.set(commandFile.data.name, commandFile)
            if (commandFile.data.aliases)
              commandFile.alias = true && commandFile.data.aliases.map((a: string) => { SuperChan.interactions.commands.set(a, commandFile)})
            ...
            SuperChan.interactions.errors.push({ type: "command", file: file, error: err })
        }
    })
}